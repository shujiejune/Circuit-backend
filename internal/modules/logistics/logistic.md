...
每个小节描述的功能应写入对应文件，可在同一文件中定义 Repository、Service 和 Handler 的最小实现。
...
1. **数据模型与仓库层**
   ...
   ```
   `models.Machine` 结构体可放在 `internal/models`，字段与 `machines` 表对应。
...
4. **单元测试**
   ```text
   示例测试: TestComputeRoute
       构造订单与假定的 Maps API 响应
       调用 ComputeRoute
       验证路线记录已保存
   ```
...
4. **测试**
   ```text
   示例测试: TestReportTracking
       提交位置上报
       查询数据库验证记录存在
   ```

---
按照以上任务逐步实现，即可在 `internal/modules/logistics` 中完成物流模块，与现有用户及订单模块协同工作。
